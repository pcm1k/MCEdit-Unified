...
class CameraViewport(GLViewport):
    ...
    def editMonsterSpawner(self, point):
        ...
        mobNames = {}
        for mob in pymclevel.Entity.monsters:
            name = pymclevel.Entity.getName(mob)
            if name is not None:
                mobNames[mob] = _(name)
...
#            tileEntity["EntityId"] = pymclevel.TAG_String(mcedit_defs.get(mobs[0], mobs[0]))
        ...
        mobTable.row_data = lambda i: (mobNames.get(mobs[i], mobs[i]),)
    ...
    def editSign(self, point):
        ...
        for l, f in zip(linekeys, lineFields):
            ...
            # Double quotes handling for old sign text format.
            if f.value == 'null':
                f.value = fmt
...
#             Button("Select", action=selectButtonAction)
    ...
    def _drawSkyBackground(self):
        ...
        if self.editor.level.dimNo == pymclevel.infiniteworld.DIM_NETHER:
            colors = numpy.array([0x90, 0x00, 0x00, 0xff,
                                  0x90, 0x00, 0x00, 0xff,
                                  0x90, 0x00, 0x00, 0xff,
                                  0x90, 0x00, 0x00, 0xff, ], dtype='uint8')
        elif self.editor.level.dimNo == pymclevel.infiniteworld.DIM_END:
